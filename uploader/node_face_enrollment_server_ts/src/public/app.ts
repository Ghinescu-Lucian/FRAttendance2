...
function validateInputs(): EnrollmentInputs {
...
async function uploadImage(blob: Blob, frameIndex: number): Promise<UploadFaceResponse> {
  ...
  const data = (await response.json().catch(() => null)) as UploadFaceResponse | null;
...
async function completeEnrollment(): Promise<CompleteEnrollmentResponse> {
  const { studentId, personName } = validateInputs();
  ...
  return (await response.json()) as CompleteEnrollmentResponse;
}
...
async function captureSequence(): Promise<void> {
  ...
  try {
    ...
    setStatus(
      `Done. Server sees ${summary.savedImages} image(s) for ${summary.name}. Saved to: ${summary.uploadDir}`
    );
  } catch (err) {
    const error = err as Error;
    console.error(error);
