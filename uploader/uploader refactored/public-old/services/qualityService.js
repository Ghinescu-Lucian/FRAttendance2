...
export class QualityService {
    ...
    message(q) {
        if (q.blur < QUALITY_RULES.minBlur)
            return "Image is blurry. Hold the phone steady.";
        if (q.brightness < QUALITY_RULES.minBrightness)
            return "Image is too dark. Add light.";
        if (q.brightness > QUALITY_RULES.maxBrightness)
            return "Image is too bright.";
        if (q.faceHeightRatio < QUALITY_RULES.minFaceHeightRatio)
            return "Move closer.";
        if (q.faceHeightRatio > QUALITY_RULES.maxFaceHeightRatio)
            return "Move slightly farther.";
        if (!q.centered)
            return "Center your face inside the green contour.";
        return "Good image.";
    }
    getImageDataForBox(sourceCanvas, box) {
