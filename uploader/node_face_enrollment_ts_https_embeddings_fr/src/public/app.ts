...
function installPoseTextOverlayOnly(): void {
  ...
  let overlay = document.getElementById("poseTextOverlayOnly") as HTMLDivElement | null;
  if (!overlay) {
...
interface SFaceEmbeddingPayload {
  version: number;
  studentId: string;
  name: string;
  createdAt: string;
...
function buildSFacePayload(studentId: string, personName: string, embeddings: CapturedEmbedding[]): SFaceEmbeddingPayload {
  return {
    version: 2,
    studentId,
    name: personName,
...
function assertValidSFacePayload(payload: SFaceEmbeddingPayload): void {
