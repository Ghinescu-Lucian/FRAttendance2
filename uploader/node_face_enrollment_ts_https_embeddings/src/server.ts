...
app.get("/api/embeddings", (_req: Request, res: Response) => {
  const files = fs
    ...
    .filter((file) => file.endsWith(".json") && file.includes("_embeddings_"));
  ...
  for (const file of files) {
    ...
    try {
      ...
      const captures = parsed.captures
        .filter((capture: any) => Array.isArray(capture.descriptor))
        .map((capture: any) => ({
          pose: String(capture.pose || "unknown"),
          label: String(capture.label || capture.pose || "unknown"),
          descriptor: capture.descriptor.map((value: unknown) => Number(value)),
      ...
      people.push({
        name: String(parsed.name || file.replace(/_embeddings_\d+\.json$/i, "")),
        file,
        captures,
      });
    } catch (error) {
      console.warn(`Skipping invalid embedding file: ${fullPath}`, error);
    }
  }
