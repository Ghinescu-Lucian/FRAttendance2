...
# Moodle integration.
# Keep USE_MOODLE_API = True when this script is used as the classroom camera station.
...
MOODLE_STATION_SOURCE = "opencv-yunet-sface-station"
MOODLE_REFRESH_SECONDS = 30
UNKNOWN_MOODLE_COOLDOWN_SECONDS = 15
MOODLE_HTTP_TIMEOUT_SECONDS = 10
UNKNOWN_THUMBNAIL_JPEG_QUALITY = 80
UNKNOWN_THUMBNAIL_MAX_WIDTH = 360
...
def http_json_get(url, headers=None):
    ...
    try:
        with urllib.request.urlopen(request, timeout=MOODLE_HTTP_TIMEOUT_SECONDS) as response:
...
def main():
    ...
    while True:
        ...
        status += f" | profile {ALGORITHM_PROFILE}"
