...
try {
    ...
    if ($existing) {
        ...
        $DB->update_record('faceattendance_embeddings', $record);
        $embeddingid = $existing->id;
        $saved = 'updated';
    } else {
        $record->timecreated = $now;
        $embeddingid = $DB->insert_record('faceattendance_embeddings', $record);
        $saved = 'created';
    }
    ...
    faceattendance_json_response([
        ...
        'uploadDir' => 'Moodle database table mdl_faceattendance_embeddings',
    ]);
} catch (Throwable $e) {
