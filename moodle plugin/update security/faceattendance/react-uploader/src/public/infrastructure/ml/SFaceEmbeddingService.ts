import { ORT_WASM_PATH, SFACE_MODEL_URL, SFACE_REFERENCE_POINTS, SFACE_SIZE } from "../../shared/appConfig";
import { Point2D } from "../../domain/types";
import { averagePoint, l2Normalize } from "../../shared/math";
...
export class SFaceEmbeddingService {
  ...
  async extract(frameCanvas: HTMLCanvasElement, result: any): Promise<number[]> {
    if (!this.session) throw new Error("SFace ONNX session is not loaded.");
    ...
    const raw = Array.from(outputs[outputName].data as Float32Array);

    if (raw.length !== 128) throw new Error(`SFace descriptor length is ${raw.length}, expected 128.`);
    return l2Normalize(raw);
  }

  private alignFace(sourceCanvas: HTMLCanvasElement, result: any): HTMLCanvasElement {
  ...
  private getSourcePoints(result: any): Point2D[] {
  ...
  private getSimilarityTransform(src: Point2D[], dst: Point2D[]): [number, number, number, number, number, number] {
  ...
  private canvasToInputTensor(alignedCanvas: HTMLCanvasElement): any {
    const ctx = alignedCanvas.getContext("2d");
    if (!ctx) throw new Error("Could not create aligned face canvas context.");
