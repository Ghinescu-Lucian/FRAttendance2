...
export class EnrollmentController {
  ...
  private async refreshCameras(activeDeviceId?: string | null): Promise<void> {
    try {
      ...
      this.view.setStatus(devices.length ? `Found ${devices.length} camera(s). Select the desired camera, then press Start camera.` : "No camera found. Check browser permissions and HTTPS/localhost.");
    } catch (err) {
  ...
  private async switchCameraIfRunning(): Promise<void> {
    ...
    this.camera.stop();
      ...
      this.view.setStatus("Camera switched. Put your face inside the face guide.");
    } catch (err) {
  ...
  async startCamera(): Promise<void> {
    try {
      ...
      this.view.startGuidance(CAPTURE_STEPS[0]);
      this.view.setStatus(`Camera started. SFace embeddings-only build: ${ENROLLMENT_APP_BUILD}. Put your face inside the face guide.`);
    } catch (err) {
      const error = err as Error;
      console.error(error);
  ...
  private async captureStep(identity: StudentIdentity, step: CaptureStep, stepIndex: number): Promise<CapturedEmbedding> {
    ...
    while (Date.now() < timeoutAt) {
      ...
      if (!result) {
        ...
        const message = "No face detected. Put your face inside the face guide.";
        this.view.setStatus(message);
        this.speech.setInstruction(message, `${step.pose}:no-face`);
        this.speech.remindIfNeeded();
      ...
      if (qualityOk && poseOk) {
        ...
        this.speech.setInstruction("Hold still.", `${step.pose}:hold-still`);
      } else {
        stableGoodFrames = 0;
        ...
        this.view.setStatus(message);
        this.speech.setInstruction(message, key);
        this.speech.remindIfNeeded();
      }
