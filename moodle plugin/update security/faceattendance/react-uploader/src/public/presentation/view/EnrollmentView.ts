...
const ENROLLMENT_CAMERA_STORAGE_KEY = "faceattendance.enrollment.cameraDeviceId";
...
export class EnrollmentView {
  ...
  readonly refreshCamerasBtn = this.required<HTMLButtonElement>("refreshCamerasBtn");
  ...
  rememberSelectedCamera(deviceId?: string | null): void {
    ...
    if (selected) localStorage.setItem(ENROLLMENT_CAMERA_STORAGE_KEY, selected);
  }

  setCameraOptions(devices: Array<{ deviceId: string; label: string }>, activeDeviceId?: string | null): void {
    const stored = localStorage.getItem(ENROLLMENT_CAMERA_STORAGE_KEY) || "";
