...
## Security update: HMAC-signed station requests

The station no longer sends the API secret directly to Moodle. The `api_secret` value is used locally as an HMAC-SHA256 signing key. Moodle verifies `X-FaceAttendance-Station`, `X-FaceAttendance-Timestamp`, `X-FaceAttendance-Nonce`, and `X-FaceAttendance-Signature` headers and rejects replayed requests.

Use HTTPS with `verify_tls: true` in production.
