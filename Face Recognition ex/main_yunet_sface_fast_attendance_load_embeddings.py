...
def parse_embedding_json(data, known_features: Dict[str, List[np.ndarray]], fallback_name: str, source_name: str):
    ...
    2) File name contains person name, JSON is only the vector/matrix:
       [0.1, 0.2, ...]
       [[0.1, ...], [0.2, ...]]

    3) One combined database file:
       {"GhinescuLucian": [[0.1, ...], [0.2, ...]], "IuliaSocarde": [[...]]}

    4) List of records:
       [{"name": "GhinescuLucian", "embedding": [0.1, ...]}]
    """
    name_keys = ("name", "person", "person_name", "personName", "label", "student", "studentName")
    ...
    if isinstance(data, dict):
        for container_key in container_keys:
            if container_key in data:
                parse_embedding_json(data[container_key], known_features, fallback_name, source_name)
                return
        ...
        for key in embedding_keys:
            if key in data:
                ...
                if is_embedding_vector(value):
                    add_embedding(known_features, person_name or fallback_name, value, source_name)
                    return
                if is_embedding_matrix(value):
                    for embedding in value:
                        add_embedding(known_features, person_name or fallback_name, embedding, source_name)
                    return
        ...
        for key, value in data.items():
            if is_embedding_vector(value):
                ...
                parse_embedding_json(value, known_features, str(key), source_name)
