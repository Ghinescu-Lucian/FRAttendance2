...
WINDOW_NAME = "Face Recognition Center Lock"
...
CENTER_SMOOTHING = 0.25   # Higher than before -> centers faster on the person
ZOOM_SMOOTHING = 0.18
UNKNOWN_LABEL = "Unknown"
MOTION_MIN_AREA = 1800
PHOTO_COOLDOWN_SECONDS = 8
FREEZE_ON_FIRST_KNOWN_LABEL = True
...
class AutoZoomController:
    ...
    def _calculate_target(self, frame_shape, target_box=None, target_type=None):
    ...
    def freeze_on_target(self, frame_shape, target_box, target_type, name=None):
        frame_h, frame_w = frame_shape[:2]
    ...
    def update(self, frame_shape, target_box=None, target_type=None):
        frame_h, frame_w = frame_shape[:2]
...
def main():
    ...
    while True:
        ...
        if zoomer.frozen:
            # Keep view steady after the first known label.
            ...
            if primary_face is not None:
                ...
                if primary_face["is_known"] and FREEZE_ON_FIRST_KNOWN_LABEL:
                    person_name = primary_face["name"]
                    zoomer.freeze_on_target(frame.shape, target_box, target_type, name=person_name)
                    crop_params = zoomer.get_crop_params(frame_w, frame_h)

                    now = time.time()
                    if now - last_capture_time_by_name[person_name] >= PHOTO_COOLDOWN_SECONDS:
                        zoomed_clean = AutoZoomController.apply_zoom(frame, crop_params)
                        saved_path = save_person_photo(person_name, zoomed_clean)
                        last_capture_time_by_name[person_name] = now
                        print(f"Saved photo for {person_name}: {saved_path}")
                else:
        ...
        if not all_faces and motion_box is not None and not zoomer.frozen:
        ...
        if zoomer.frozen:
            status += f" | CAMERA LOCKED on {zoomer.locked_name or 'recognized person'}"
        elif target_type == "face":
            status += " | Centering on face"
