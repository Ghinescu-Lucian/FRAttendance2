    ...
    while True:
        ...
        if target_face is not None:
            ...
            if is_known and stable_count >= STABLE_FRAMES_REQUIRED:
                ...
                if SAVE_PHOTO_WHEN_KNOWN_STABLE and saved_count_by_name[name] < MAX_PHOTOS_PER_PERSON:
                    now = time.time()
                    ...
                    if now - last_capture_time_by_name[name] >= PHOTO_COOLDOWN_SECONDS:
                        ...
                        print(
                            f"Saved photo {saved_count_by_name[name]}/{MAX_PHOTOS_PER_PERSON} "
                            f"for {name}: {image_path}"
                        )
        ...
        if key in (ord("r"), ord("R")):
            ...
            stable_count = 0
            last_candidates = []
            # The saved photo count is intentionally NOT reset.
            # This keeps the rule: maximum two saved photos per person for this run.
            print("Reset followed person. Saved-photo counters were kept.")
