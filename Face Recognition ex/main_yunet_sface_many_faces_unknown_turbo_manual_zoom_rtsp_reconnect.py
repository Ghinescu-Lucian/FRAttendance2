...
# These options are used by OpenCV's FFmpeg backend when opening RTSP cameras.
# They reduce buffering and force TCP, which is usually more reliable on normal LANs.
os.environ.setdefault(
    "OPENCV_FFMPEG_CAPTURE_OPTIONS",
    "rtsp_transport;tcp|fflags;nobuffer|flags;low_delay|stimeout;5000000|max_delay;500000",
)
...
#
# Run:
#   py main_yunet_sface_many_faces_unknown_turbo_manual_zoom_rtsp_reconnect.py --camera-ip 192.168.0.138
# ============================================================
...
# =========================
# Camera source:
#   "rtsp"  = HiLook/Hikvision IP camera over LAN
#   "webcam" = normal USB/laptop camera using CAMERA_INDEX
CAMERA_SOURCE = os.environ.get("CAMERA_SOURCE", "rtsp").strip().lower()

# Used only when CAMERA_SOURCE = "webcam".
CAMERA_INDEX = int(os.environ.get("CAMERA_INDEX", "0"))

# Used when CAMERA_SOURCE = "rtsp".
# You can either provide CAMERA_RTSP_URL directly, or provide IP/user/password/channel below.
# Examples:
#   rtsp://admin:password@192.168.0.138:554/Streaming/Channels/101  main stream
#   rtsp://admin:password@192.168.0.138:554/Streaming/Channels/102  sub stream
CAMERA_RTSP_URL = os.environ.get("CAMERA_RTSP_URL", "").strip()
CAMERA_IP = os.environ.get("CAMERA_IP", "192.168.0.138").strip()
CAMERA_USERNAME = os.environ.get("CAMERA_USERNAME", "admin").strip()
CAMERA_PASSWORD = os.environ.get("CAMERA_PASSWORD", "")
CAMERA_RTSP_PORT = int(os.environ.get("CAMERA_RTSP_PORT", "554"))
CAMERA_CHANNEL = os.environ.get("CAMERA_CHANNEL", "101").strip()
RTSP_TRANSPORT = os.environ.get("RTSP_TRANSPORT", "tcp").strip().lower()

# If RTSP decoding drops frames/corrupts packets, keep the program alive and reopen the stream.
RTSP_READ_FAIL_RECONNECT_AFTER = int(os.environ.get("RTSP_READ_FAIL_RECONNECT_AFTER", "10"))
RTSP_RECONNECT_SLEEP_SECONDS = float(os.environ.get("RTSP_RECONNECT_SLEEP_SECONDS", "1.0"))
...
MANUAL_ZOOM_STEP = 0.10
MANUAL_PAN_STEP_RATIO = 0.05
...
def apply_runtime_camera_args():
    """Allow changing the camera without editing this file."""
    global CAMERA_SOURCE, CAMERA_INDEX, CAMERA_RTSP_URL, CAMERA_IP
    global CAMERA_USERNAME, CAMERA_PASSWORD, CAMERA_RTSP_PORT, CAMERA_CHANNEL
    global RTSP_TRANSPORT, RTSP_READ_FAIL_RECONNECT_AFTER, RTSP_RECONNECT_SLEEP_SECONDS, EMBEDDINGS_SOURCE

    parser = argparse.ArgumentParser(
        description="YuNet + SFace attendance script with webcam or HiLook/Hikvision RTSP camera input."
    )
    parser.add_argument(
        "--camera-source",
        choices=["rtsp", "webcam"],
        default=CAMERA_SOURCE,
        help="Use 'rtsp' for the HiLook IP camera, or 'webcam' for a USB/laptop camera.",
    )
    parser.add_argument("--camera-index", type=int, default=CAMERA_INDEX, help="Webcam index when --camera-source webcam is used.")
    parser.add_argument("--rtsp-url", default=CAMERA_RTSP_URL, help="Full RTSP URL. If set, it overrides IP/user/password/channel.")
    parser.add_argument("--camera-ip", default=CAMERA_IP, help="Camera IP address, for example 192.168.0.138.")
    parser.add_argument("--camera-username", default=CAMERA_USERNAME, help="Camera web/RTSP username. Usually admin.")
    parser.add_argument(
        "--camera-password",
        default=CAMERA_PASSWORD,
        help="Camera web/RTSP password. If omitted for RTSP, the script asks for it safely.",
    )
    parser.add_argument("--camera-port", type=int, default=CAMERA_RTSP_PORT, help="RTSP port. Usually 554.")
    parser.add_argument(
        "--camera-channel",
        default=CAMERA_CHANNEL,
        help="HiLook/Hikvision stream channel: 101 = main stream, 102 = substream.",
    )
    parser.add_argument(
        "--rtsp-transport",
        choices=["tcp", "udp"],
        default=RTSP_TRANSPORT if RTSP_TRANSPORT in {"tcp", "udp"} else "tcp",
        help="RTSP transport. TCP is usually more stable on LAN.",
    )
    parser.add_argument(
        "--rtsp-read-fail-reconnect-after",
        type=int,
        default=RTSP_READ_FAIL_RECONNECT_AFTER,
        help="For RTSP only: reopen the stream after this many consecutive failed frame reads.",
    )
    parser.add_argument(
        "--rtsp-reconnect-sleep",
        type=float,
        default=RTSP_RECONNECT_SLEEP_SECONDS,
        help="For RTSP only: seconds to wait before reopening a broken RTSP stream.",
    )
    parser.add_argument("--embeddings", default=EMBEDDINGS_SOURCE, help="Embeddings folder or JSON file.")

    args = parser.parse_args()

    CAMERA_SOURCE = args.camera_source
    CAMERA_INDEX = args.camera_index
    CAMERA_RTSP_URL = args.rtsp_url.strip()
    CAMERA_IP = args.camera_ip.strip()
    CAMERA_USERNAME = args.camera_username.strip()
    CAMERA_PASSWORD = args.camera_password
    CAMERA_RTSP_PORT = args.camera_port
    CAMERA_CHANNEL = args.camera_channel.strip()
    RTSP_TRANSPORT = args.rtsp_transport
    RTSP_READ_FAIL_RECONNECT_AFTER = max(1, int(args.rtsp_read_fail_reconnect_after))
    RTSP_RECONNECT_SLEEP_SECONDS = max(0.0, float(args.rtsp_reconnect_sleep))
    EMBEDDINGS_SOURCE = args.embeddings

    if CAMERA_SOURCE == "rtsp" and not CAMERA_RTSP_URL and not CAMERA_PASSWORD:
        CAMERA_PASSWORD = getpass.getpass("Camera admin password: ")


def build_rtsp_url():
    """Build the HiLook/Hikvision RTSP URL from the configured values."""
    if CAMERA_RTSP_URL:
        return CAMERA_RTSP_URL

    if not CAMERA_IP:
        raise RuntimeError("CAMERA_IP is empty. Set --camera-ip or CAMERA_IP.")

    if not CAMERA_USERNAME:
        raise RuntimeError("CAMERA_USERNAME is empty. Set --camera-username or CAMERA_USERNAME.")

    if not CAMERA_PASSWORD:
        raise RuntimeError(
            "CAMERA_PASSWORD is empty. Pass --camera-password, set CAMERA_PASSWORD, "
            "or set --rtsp-url to the complete RTSP URL."
        )

    username = quote(CAMERA_USERNAME, safe="")
    password = quote(CAMERA_PASSWORD, safe="")
    channel = CAMERA_CHANNEL or "101"

    return (
        f"rtsp://{username}:{password}@{CAMERA_IP}:{CAMERA_RTSP_PORT}"
        f"/Streaming/Channels/{channel}"
    )


def mask_camera_url(url):
    """Hide the password before printing the RTSP URL."""
    return re.sub(r"(rtsp://[^:/@]+:)([^@]+)(@)", r"\1***\3", str(url))


def open_camera():
    if CAMERA_SOURCE == "rtsp":
        # Refresh the OpenCV/FFmpeg options in case the user selected UDP/TCP using CLI args.
        os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = (
            f"rtsp_transport;{RTSP_TRANSPORT}|fflags;nobuffer|flags;low_delay|"
            f"stimeout;5000000|max_delay;500000"
        )

        rtsp_url = build_rtsp_url()
        print(f"Opening RTSP camera: {mask_camera_url(rtsp_url)}")

        cap = cv2.VideoCapture(rtsp_url, cv2.CAP_FFMPEG)
        cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)

        if not cap.isOpened():
            print("[WARN] FFmpeg backend failed. Trying OpenCV default backend...")
            cap.release()
    ...
    if CAMERA_SOURCE != "webcam":
        raise RuntimeError(f"Unsupported CAMERA_SOURCE: {CAMERA_SOURCE}. Use 'rtsp' or 'webcam'.")

    print(f"Opening local webcam index: {CAMERA_INDEX}")
...
def reopen_camera(old_cap=None):
    """Release and reopen the camera stream without killing the attendance program."""
    ...
    if CAMERA_SOURCE == "rtsp":
        print(f"[RTSP] Reconnecting in {RTSP_RECONNECT_SLEEP_SECONDS:.1f}s...")
        time.sleep(RTSP_RECONNECT_SLEEP_SECONDS)
        new_cap = open_camera()
        if new_cap.isOpened():
            print("[RTSP] Reconnected.")
        else:
            print("[RTSP] Reconnect failed; will retry.")
        return new_cap
...
def main():
    ...
    if not cap.isOpened():
        if CAMERA_SOURCE == "rtsp":
            raise RuntimeError(
                "Could not open RTSP camera. Check the camera IP, admin password, RTSP port 554, "
                f"and stream channel. Tried: {mask_camera_url(build_rtsp_url())}"
            )
    ...
    if CAMERA_SOURCE == "rtsp":
        print(f"RTSP auto-reconnect after {RTSP_READ_FAIL_RECONNECT_AFTER} consecutive failed frame reads.\n")
    else:
        print()
    ...
    while True:
        ...
        if not ret or frame is None:
            ...
            if CAMERA_SOURCE == "rtsp":
                print(
                    f"[WARN] Could not read RTSP frame "
                    f"({consecutive_read_failures}/{RTSP_READ_FAIL_RECONNECT_AFTER})."
                )
                if consecutive_read_failures >= RTSP_READ_FAIL_RECONNECT_AFTER:
                    print("[RTSP] Stream read failed repeatedly. Reopening the camera stream.")
