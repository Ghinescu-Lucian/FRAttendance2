...
class ResilientNetworkCapture:
    """Non-blocking RTSP/HTTP/video-file reader with reconnect support.

    OpenCV's VideoCapture.read() can block for a long time on RTSP streams.
    This wrapper keeps that blocking call in a daemon reader thread and exposes
    a fast read() method to the recognition loop. If the stream stalls, the UI
    and recognition loop remain responsive while the reader reconnects.
    """
    ...
    def _configure_ffmpeg_options(self) -> None:
        # Respect a user-supplied value. This is useful for debugging special cameras.
        if os.environ.get("OPENCV_FFMPEG_CAPTURE_OPTIONS"):
            return
        lower = self.source.lower()
        if lower.startswith("rtsp://"):
            # TCP is usually more stable than UDP on Wi-Fi/LAN surveillance cameras.
            # stimeout is in microseconds. It prevents very long blocking reads.
    ...
    def isOpened(self) -> bool:
        # The wrapper is usable while its reader thread is alive; frames may arrive later.
...
class StationWorker(threading.Thread):
    ...
    def _open_capture(self):
        ...
        # Use a resilient reader so RTSP stalls do not freeze the recognition loop.
    ...
    def run(self) -> None:
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if not ok or frame is None:
                    # For RTSP this usually means the reader is reconnecting or
                    # waiting for a fresh frame. Keep the UI responsive.
                    time.sleep(0.04)
                    continue
...
class DesktopStationApp(tk.Tk):
    ...
    def _build_camera_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Checkbutton(side, text="Enable zoom before recognition", variable=self.zoom_enabled, command=self.on_zoom_enabled_changed).grid(row=13, column=0, columnspan=3, sticky="w", pady=(6, 0))
    ...
    def on_zoom_enabled_changed(self) -> None:
        if self.zoom_enabled.get() and self.zoom_factor.get() <= 1.001:
    ...
    def push_controls(self) -> None:
        ...
        if zoom_enabled and zoom_factor <= 1.001:
            zoom_factor = 1.5
