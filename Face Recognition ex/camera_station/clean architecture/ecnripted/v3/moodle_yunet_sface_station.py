...
MOODLE_STATION_ID = os.environ.get("FACEATTENDANCE_STATION_ID", MOODLE_STATION_SOURCE)
...
MOODLE_STATION_PRIVATE_KEY_PATH = os.environ.get("FACEATTENDANCE_STATION_PRIVATE_KEY_PATH", "station_private_key.b64")
MOODLE_STATION_PRIVATE_KEY_B64 = os.environ.get("FACEATTENDANCE_STATION_PRIVATE_KEY_B64", "")
...
ALGORITHM_PROFILES = {
    ...
    "walkthrough_realtime": {
        ...
        "skip_resolved_recognition": True,
        "max_recognitions_per_frame": 24,
        # Keep unknown-review capture enabled in the desktop application.
        # 0 disables saving unknown faces entirely; this profile is commonly used
        # during live walkthrough tests where manual labelling is still needed.
        "unknown_registry_max_updates_per_frame": 2,
...
def apply_runtime_options():
    ...
    """
    global MOODLE_BASE_URL, MOODLE_CMID, MOODLE_API_SECRET, MOODLE_STATION_SOURCE, MOODLE_STATION_ID
    global CAMERA_INDEX, MOODLE_ALGORITHM_PROFILE, ALGORITHM_PROFILE
    global MOODLE_REFRESH_SECONDS, UNKNOWN_MOODLE_COOLDOWN_SECONDS, MOODLE_HTTP_TIMEOUT_SECONDS, MOODLE_VERIFY_TLS, MOODLE_STATION_PRIVATE_KEY_PATH, MOODLE_STATION_PRIVATE_KEY_B64
    ...
    parser.add_argument("--station-id", default=None, help="Stable station id used in HMAC authentication headers. Defaults to source.")
    parser.add_argument("--station-private-key", default=None, help="Path to the base64 station private key used to decrypt Moodle encrypted embeddings.")
    ...
    # Use parse_known_args so the GUI can pass its own arguments without breaking config loading.
    ...
    def resolve_config_path(config_arg):
        ...
        if config_arg:
            ...
            # Be tolerant: older instructions sometimes used station.json,
            # while the newer package uses station_config.json.
        ...
        if config_arg:
            checked = "\n".join(f"  - {c.resolve()}" for c in ordered)
            raise RuntimeError("Config file was explicitly requested but not found. Checked:\n" + checked)
        checked = "\n".join(f"  - {c.resolve()}" for c in ordered)
        print("[CONFIG] No station_config.json/station.json found. Checked:\n" + checked)
        return None
    ...
    if config_path is not None:
        ...
        if "api_secret" in config and str(config.get("api_secret", "")).strip():
            print("[CONFIG] Moodle API secret: loaded from config")
        else:
            print("[CONFIG] Moodle API secret: NOT found in config")
    else:
    ...
    MOODLE_API_SECRET = str(pick("api_secret", args.secret, MOODLE_API_SECRET)).strip()
    ...
    MOODLE_STATION_ID = str(pick("station_id", args.station_id, config.get("stationid", MOODLE_STATION_SOURCE)))
    ...
    MOODLE_STATION_PRIVATE_KEY_PATH = str(pick("station_private_key_path", args.station_private_key, MOODLE_STATION_PRIVATE_KEY_PATH))
    MOODLE_STATION_PRIVATE_KEY_B64 = str(config.get("station_private_key_b64", MOODLE_STATION_PRIVATE_KEY_B64) or MOODLE_STATION_PRIVATE_KEY_B64)
    ...
    print(f"  Station ID: {MOODLE_STATION_ID}")
...
def _moodle_request_target(url):
...
def _force_load_station_config_for_signing_if_needed():
    """Last-resort config loader used before HMAC signing.

    The desktop GUI normally calls apply_runtime_options() after importing this
    module. If a user accidentally runs an older GUI launcher, imports this
    module directly, or starts the script from another working directory, the
    HMAC secret can remain at its default value. This function reads the config
    directly from common locations so the Moodle API secret is still loaded.
    """
    global MOODLE_BASE_URL, MOODLE_CMID, MOODLE_API_SECRET, MOODLE_STATION_SOURCE, MOODLE_STATION_ID
    global MOODLE_VERIFY_TLS, MOODLE_HTTP_TIMEOUT_SECONDS, MOODLE_ALGORITHM_PROFILE, ALGORITHM_PROFILE
    global MOODLE_STATION_PRIVATE_KEY_PATH, MOODLE_STATION_PRIVATE_KEY_B64

    if MOODLE_API_SECRET and MOODLE_API_SECRET not in (
        "change-this-secret",
        "CHANGE_ME_USE_LONG_RANDOM_SECRET_FROM_MOODLE",
    ):
        return

    candidates = []
    env_config = os.environ.get("FACEATTENDANCE_CONFIG")
    if env_config:
    ...
    for candidate in candidates:
        ...
        if not candidate.exists():
            continue
        try:
            config = _load_json_config(candidate)
        except Exception as exc:
            print(f"[CONFIG] Could not read fallback config {candidate}: {exc}")
            continue

        secret = str(config.get("api_secret", "") or "").strip()
        if secret:
            MOODLE_API_SECRET = secret
            MOODLE_BASE_URL = str(config.get("moodle_base_url", MOODLE_BASE_URL)).rstrip("/")
            try:
                MOODLE_CMID = int(config.get("cmid", MOODLE_CMID))
            except Exception:
                pass
            MOODLE_STATION_SOURCE = str(config.get("source", MOODLE_STATION_SOURCE))
            MOODLE_STATION_ID = str(config.get("station_id", config.get("stationid", MOODLE_STATION_SOURCE)))
            MOODLE_VERIFY_TLS = bool_config_value(config.get("verify_tls", MOODLE_VERIFY_TLS), MOODLE_VERIFY_TLS)
            try:
                MOODLE_HTTP_TIMEOUT_SECONDS = int(float(config.get("http_timeout_seconds", MOODLE_HTTP_TIMEOUT_SECONDS)))
            except Exception:
                pass
            MOODLE_STATION_PRIVATE_KEY_PATH = str(config.get("station_private_key_path", MOODLE_STATION_PRIVATE_KEY_PATH))
            MOODLE_STATION_PRIVATE_KEY_B64 = str(config.get("station_private_key_b64", MOODLE_STATION_PRIVATE_KEY_B64) or MOODLE_STATION_PRIVATE_KEY_B64)
            profile = str(config.get("profile", MOODLE_ALGORITHM_PROFILE) or MOODLE_ALGORITHM_PROFILE)
            if profile in ALGORITHM_PROFILES:
                MOODLE_ALGORITHM_PROFILE = profile
                ALGORITHM_PROFILE = profile
            print(f"[CONFIG] Fallback loaded Moodle API secret from: {candidate}")
            return

def _moodle_signed_headers(method, url, body=b""):
    """Create HMAC headers. The raw secret is never transmitted to Moodle."""
    _force_load_station_config_for_signing_if_needed()
    if not MOODLE_API_SECRET or MOODLE_API_SECRET in ("change-this-secret", "CHANGE_ME_USE_LONG_RANDOM_SECRET_FROM_MOODLE"):
        checked = ", ".join([
            str(Path.cwd() / "station_config.json"),
            str(Path.cwd() / "station.json"),
            str(SCRIPT_DIR / "station_config.json"),
            str(SCRIPT_DIR / "station.json"),
        ])
        raise RuntimeError(
            "Missing Moodle API secret after fallback config load. "
            "Set api_secret in station_config.json/station.json or FACEATTENDANCE_API_SECRET. "
            f"Checked: {checked}"
        )
    ...
    station_id = str(MOODLE_STATION_ID or MOODLE_STATION_SOURCE)
    body_hash = hashlib.sha256(body).hexdigest()
    canonical = "\n".join([
        method.upper(),
        _moodle_request_target(url),
        timestamp,
        nonce,
        station_id,
        body_hash,
    ...
    signature = hmac.new(
        MOODLE_API_SECRET.encode("utf-8"),
        canonical.encode("utf-8"),
        hashlib.sha256,
    ).hexdigest()
    return {
        "X-FaceAttendance-Station": station_id,
        "X-FaceAttendance-Timestamp": timestamp,
        "X-FaceAttendance-Nonce": nonce,
        "X-FaceAttendance-Signature": signature,
    }
...
def http_json_get(url, headers=None):
    ...
    request_headers.update(_moodle_signed_headers("GET", url, b""))
...
_MOODLE_STATION_PRIVATE_KEY = None


def _load_station_private_key_bytes() -> bytes:
    raw_b64 = str(MOODLE_STATION_PRIVATE_KEY_B64 or "").strip()
    if raw_b64:
        return base64.b64decode(raw_b64)

    key_path = Path(str(MOODLE_STATION_PRIVATE_KEY_PATH or "")).expanduser()
    if not key_path.is_absolute():
        key_path = SCRIPT_DIR / key_path
    if not key_path.exists():
        raise RuntimeError(
            "Moodle returned encrypted embeddings, but no station private key was found. "
            f"Expected: {key_path}. Generate one with generate_station_crypto_keys.py "
            "and copy the printed public key into the Moodle activity settings."
        )
    return base64.b64decode(key_path.read_text(encoding="utf-8").strip())


def _station_private_key():
    global _MOODLE_STATION_PRIVATE_KEY
    if _MOODLE_STATION_PRIVATE_KEY is not None:
        return _MOODLE_STATION_PRIVATE_KEY
    try:
        from nacl.public import PrivateKey
    except Exception as exc:
        raise RuntimeError(
            "Encrypted embeddings require PyNaCl. Install it with: py -m pip install PyNaCl"
        ) from exc

    raw = _load_station_private_key_bytes()
    if len(raw) != 32:
        raise RuntimeError("Invalid station private key length. Expected 32 raw bytes encoded as base64.")
    _MOODLE_STATION_PRIVATE_KEY = PrivateKey(raw)
    return _MOODLE_STATION_PRIVATE_KEY


def decrypt_moodle_embedding_envelope(envelope: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(envelope, dict) or not envelope.get("faceattendanceEncryptedEmbedding"):
        raise RuntimeError("Invalid encrypted embedding envelope from Moodle.")
    if envelope.get("scheme") != "libsodium-sealed-box-x25519-xsalsa20poly1305":
        raise RuntimeError(f"Unsupported encrypted embedding scheme: {envelope.get('scheme')}")
    ciphertext_b64 = envelope.get("ciphertext")
    if not isinstance(ciphertext_b64, str) or not ciphertext_b64.strip():
        raise RuntimeError("Encrypted embedding envelope does not contain ciphertext.")
    ...
    plaintext = SealedBox(_station_private_key()).decrypt(ciphertext)
    payload = json.loads(plaintext.decode("utf-8"))
    if not isinstance(payload, dict):
        raise RuntimeError("Decrypted embedding payload is not a JSON object.")
    return payload


def expand_moodle_face_record(face: Dict[str, Any]):
    """Yield concrete descriptor records from either plaintext or encrypted Moodle face records."""
    userid = face.get("userid") or face.get("studentId")
    if userid is None:
        return

    encrypted = face.get("encryptedEmbedding")
    if isinstance(encrypted, dict):
        payload = decrypt_moodle_embedding_envelope(encrypted)
        ...
        for index, capture in enumerate(captures):
            ...
            yield {
                "userid": userid,
                "name": face.get("name") or str(userid),
                "descriptor": descriptor,
                "pose": capture.get("pose") or "unknown",
                "label": capture.get("label") or f"sample_{index + 1}",
                "sourceFile": face.get("sourceFile") or "moodle_encrypted_embedding",
            }
        return

    descriptor = face.get("descriptor")
    if is_embedding_vector(descriptor):
        yield face
...
def http_json_post(url, payload, headers=None):
    ...
    if headers:
        request_headers.update(headers)
    request_headers.update(_moodle_signed_headers("POST", url, body))
...
def load_known_features_from_moodle():
    ...
    encrypted_records = 0
    for face in faces:
        if isinstance(face.get("encryptedEmbedding"), dict):
            encrypted_records += 1
        for expanded in expand_moodle_face_record(face):
            userid = expanded.get("userid") or expanded.get("studentId")
            descriptor = expanded.get("descriptor")
            if userid is None or not is_embedding_vector(descriptor):
                continue

            person_key = str(userid)
            labels[person_key] = str(expanded.get("name") or person_key)
            add_embedding(known_features, person_key, descriptor, f"Moodle userid {person_key}")

    if encrypted_records:
        print(f"Decrypted {encrypted_records} encrypted Moodle embedding record(s) with the local station private key.")
