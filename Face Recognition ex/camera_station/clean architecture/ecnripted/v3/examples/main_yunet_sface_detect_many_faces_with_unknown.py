...
#
# Run:
#   py main_yunet_sface_detect_many_faces_fast_clean.py
# ============================================================
...
FULL_GRID_SEARCH_EVERY_N_FRAMES = 16

# Search zooms.
SEARCH_ZOOM_LEVELS_FAST = [1.0]
SEARCH_ZOOM_LEVELS_FULL = [1.0, 1.8, 2.6]
...
def main():
    ...
    print("Detect-many mode: full-frame display, known + unknown faces, duplicate Unknown suppression.")
    print("Press U to toggle Unknown drawing.\n")
    ...
    while True:
        ...
        for candidate in candidates:
            ...
            if is_known:
                ...
                label = f"Unknown #{unknown_draw_index}"
                if "similarity" in candidate:
                    label += f" best {candidate['similarity']:.3f}"
        ...
        if should_full_grid_search:
            status += " | periodic zoom search"
        elif should_fast_search:
            status += " | full-frame search"
        else:
            status += " | reusing previous frame result"
        ...
        cv2.putText(
            display_frame,
            "ESC = exit | R = reset tracking state | U = toggle Unknown",
            (20, frame_h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.65,
            (255, 255, 255),
