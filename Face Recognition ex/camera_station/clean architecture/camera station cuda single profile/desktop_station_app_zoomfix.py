...
def desktop_bool_config_value(value, default=False):
...
def apply_desktop_moodle_config() -> None:
    """Load Moodle connection settings for the Tk desktop station.

    The GUI imports moodle_yunet_sface_station as a module, so the normal
    command-line apply_runtime_options() function is not executed. Without this
    small bridge the station keeps the built-in defaults such as
    http://localhost/moodle, cmid=12, secret=change-this-secret.
    """
    config_path = APP_DIR / "station_config.json"
    if not config_path.exists():
        print("[CONFIG] station_config.json not found beside desktop app; using Moodle defaults/env values.")
        return
    ...
    try:
        ...
        if not isinstance(config, dict):
            raise RuntimeError("station_config.json must contain a JSON object")
    except Exception as exc:
        print(f"[CONFIG] Could not read station_config.json: {exc}")
        return

    station.MOODLE_BASE_URL = str(config.get("moodle_base_url", station.MOODLE_BASE_URL)).rstrip("/")
    station.MOODLE_CMID = int(config.get("cmid", station.MOODLE_CMID))
    station.MOODLE_API_SECRET = str(config.get("api_secret", station.MOODLE_API_SECRET))
    station.MOODLE_STATION_SOURCE = str(config.get("source", station.MOODLE_STATION_SOURCE))
    station.MOODLE_REFRESH_SECONDS = int(float(config.get("refresh_seconds", station.MOODLE_REFRESH_SECONDS)))
    station.UNKNOWN_MOODLE_COOLDOWN_SECONDS = int(float(config.get("unknown_cooldown_seconds", station.UNKNOWN_MOODLE_COOLDOWN_SECONDS)))
    station.MOODLE_HTTP_TIMEOUT_SECONDS = int(float(config.get("http_timeout_seconds", station.MOODLE_HTTP_TIMEOUT_SECONDS)))
    station.MOODLE_VERIFY_TLS = desktop_bool_config_value(config.get("verify_tls", station.MOODLE_VERIFY_TLS), station.MOODLE_VERIFY_TLS)

    profile = str(config.get("profile", getattr(station, "MOODLE_ALGORITHM_PROFILE", "fast_short")) or "fast_short").strip()
    if hasattr(station, "ALGORITHM_PROFILES") and profile in station.ALGORITHM_PROFILES:
        station.MOODLE_ALGORITHM_PROFILE = profile
        station.ALGORITHM_PROFILE = profile
    elif profile:
        print(f"[CONFIG] Unknown Moodle profile '{profile}', keeping {getattr(station, 'MOODLE_ALGORITHM_PROFILE', 'default')}.")

    print(f"[CONFIG] Desktop loaded station_config.json: {config_path}")
    print(f"[CONFIG] Moodle URL: {station.MOODLE_BASE_URL}")
    print(f"[CONFIG] Moodle cmid: {station.MOODLE_CMID}")
    print(f"[CONFIG] Verify TLS: {station.MOODLE_VERIFY_TLS}")
