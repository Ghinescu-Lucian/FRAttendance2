...
CANDIDATE_IOU_DEDUPE_THRESHOLD = 0.40
...
def dedupe_candidates(candidates, iou_threshold=CANDIDATE_IOU_DEDUPE_THRESHOLD):
    ...
    full frame, center crop, last-box crop, and grid crops overlap. Keep one
    candidate per physical face before updating attendance state.
    """
    ...
    for candidate in sorted_candidates:
        if any(intersection_over_union(candidate["box"], existing["box"]) >= iou_threshold for existing in kept):
            continue
...
def main():
    ...
    print("Entrance mode: only faces inside the yellow entrance zone are selected.")
    print("Multi-face mode: every visible known face is tracked and marked independently.\n")

    zoomer = FaceCenterZoomer()

    # One focus box is still useful for the visual zoom. Attendance is no longer
    # locked to this one face.
    focus_box = None
    lost_focus_frames = 0

    # Per-person state. This replaces the old single global stable_name/stable_count.
    ...
    while True:
        ...
        if should_fast_search or should_full_grid_search:
            ...
            # Keep all physical faces, not all duplicate crop detections.
            candidates = filter_candidates_by_entrance(candidates, frame_w, frame_h)
            candidates = dedupe_candidates(candidates)
            last_candidates = candidates

        # Choose only the display focus. This does NOT control attendance anymore.
        focus_face = choose_target_face(
            candidates,
            frame_w,
            frame_h,
            locked_name=None,
        ...
        if focus_face is not None:
            focus_box = focus_face["box"]
            lost_focus_frames = 0
        else:
            lost_focus_frames += 1
            if lost_focus_frames > MAX_LOST_FRAMES_AFTER_LOCK:
                focus_box = None
                last_candidates = []

        crop_params = zoomer.update(frame.shape, face_box=focus_box)
        display_frame = FaceCenterZoomer.apply_zoom(frame, crop_params)

        if DRAW_ENTRANCE_ZONE:
            entrance_zone = get_entrance_zone(frame_w, frame_h)
            zx1, zy1, zx2, zy2 = transform_box_to_zoomed_view(
                entrance_zone,
                crop_params,
                frame_w,
                frame_h,
        ...
        # Remove people that disappeared so their stability must be rebuilt when they re-enter.
        ...
        # Draw every detected face that is visible inside the current display crop.
        for candidate in candidates:
            if candidate.get("is_known", False):
                if not DRAW_ALL_RECOGNIZED_FACES:
                    continue
            else:
            ...
            if not box_intersects_crop(face_box, crop_params):
                continue
            ...
            is_focus = focus_face is not None and intersection_over_union(face_box, focus_face["box"]) >= 0.60

            color = (0, 255, 255) if is_focus else ((0, 255, 0) if is_known else (0, 0, 255))
            thickness = 4 if is_focus else 2

            label = name
            if "similarity" in candidate:
                label += f" {candidate['similarity']:.3f}"
        ...
        if ENTRANCE_MODE:
            status += " | Entrance mode"

        status += f" | Tracking {known_count} known / {len(candidates)} face(s)"

        if focus_face is not None:
            status += f" | Focus {focus_face.get('name', UNKNOWN_LABEL)}"
        else:
        ...
        cv2.putText(
            display_frame,
            "ESC = exit | R = reset tracking state",
            (20, frame_h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.65,
            (255, 255, 255),
