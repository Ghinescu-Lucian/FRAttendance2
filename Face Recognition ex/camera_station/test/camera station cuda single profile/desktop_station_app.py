    ...
    def run(self) -> None:
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if is_video_playback:
                    while not self.stop_event.is_set() and bool(_controls.get("playback_paused", False)):
...
class VideoRecognitionWindow(tk.Toplevel):
    ...
    def _build_ui(self) -> None:
        ...
        controls = ttk.Frame(root, style="Card.TFrame", padding=10)
        controls.grid(row=0, column=0, sticky="ew", pady=(0, 10))
        controls.columnconfigure(1, weight=1)

        ttk.Label(controls, text="Video file", style="Card.TLabel").grid(row=0, column=0, sticky="w")
        ...
        self.start_btn = ttk.Button(buttons, text="Start video FR", command=self.start_video)
        self.pause_btn = ttk.Button(buttons, text="Pause", command=self.toggle_pause, state=tk.DISABLED)
        self.stop_btn = ttk.Button(buttons, text="Stop", command=self.stop_video, state=tk.DISABLED)
        ...
        for value in (0.5, 1.0, 2.0, 4.0):
            ttk.Button(buttons, text=f"{value:g}x", width=4, command=lambda v=value: self.set_speed(v)).pack(side=tk.LEFT, padx=(0, 4))
        ...
        ttk.Label(controls, textvariable=self.status_var, style="Status.TLabel").grid(row=3, column=0, columnspan=3, sticky="w", pady=(6, 0))
        ...
        self.video_label = ttk.Label(video_card, text="Start video FR to show processed frames", anchor=tk.CENTER, style="Card.TLabel")
...
class DesktopStationApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(actions, text="Video speed", style="Muted.TLabel").pack(side=tk.LEFT, padx=(0, 4))
        ...
        self.video_pause_btn = ttk.Button(actions, text="Pause video", width=12, command=self.toggle_video_pause, state=tk.DISABLED)
        ...
        self.video_btn = ttk.Button(actions, text="Open video window", width=17, command=self.open_video_window)
    ...
    def _build_camera_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Radiobutton(side, text="Camera index", variable=self.source_kind, value="camera", command=self.on_source_kind_changed).grid(row=1, column=0, sticky="w")
        ttk.Radiobutton(side, text="RTSP/HTTP stream", variable=self.source_kind, value="stream", command=self.on_source_kind_changed).grid(row=2, column=0, sticky="w")
        ttk.Radiobutton(side, text="Video file", variable=self.source_kind, value="video_file", command=self.on_source_kind_changed).grid(row=2, column=1, columnspan=2, sticky="w")

        ttk.Label(side, text="Source", style="Card.TLabel").grid(row=3, column=0, sticky="w", pady=(12, 0))
        ttk.Label(side, text="RTSP transport", style="Card.TLabel").grid(row=3, column=1, sticky="e", padx=(8, 6), pady=(12, 0))
        ...
        self.rtsp_transport_box.grid(row=3, column=2, sticky="ew", pady=(12, 0))
        ...
        ttk.Label(side, textvariable=self.source_hint_var, style="Muted.TLabel", wraplength=340).grid(row=5, column=0, columnspan=3, sticky="w", pady=(6, 0))

        ttk.Label(side, text="Embeddings source", style="Card.TLabel").grid(row=6, column=0, sticky="w", pady=(12, 0))
        self.embeddings_source = tk.StringVar(value=str(APP_DIR / "images"))
        ttk.Entry(side, textvariable=self.embeddings_source, width=36).grid(row=7, column=0, columnspan=2, sticky="ew", pady=(4, 0))
        ttk.Button(side, text="Browse", command=self.browse_embeddings).grid(row=7, column=2, sticky="ew", padx=(6, 0), pady=(4, 0))

        ttk.Label(side, text="Profile", style="Card.TLabel").grid(row=8, column=0, sticky="w", pady=(12, 0))
        ...
        profile_box.grid(row=9, column=0, columnspan=3, sticky="ew", pady=(4, 0))

        self.use_gpu = tk.BooleanVar(value=True)
        self.use_moodle = tk.BooleanVar(value=False)
        ttk.Checkbutton(side, text="Try GPU acceleration (ONNX Runtime CUDA for SFace)", variable=self.use_gpu).grid(row=10, column=0, columnspan=3, sticky="w", pady=(12, 0))
        ttk.Checkbutton(side, text="Load roster/embeddings from Moodle API", variable=self.use_moodle).grid(row=11, column=0, columnspan=3, sticky="w", pady=(4, 0))

        ttk.Separator(side).grid(row=12, column=0, columnspan=3, sticky="ew", pady=14)
        ttk.Label(side, text="Digital zoom", style="Card.TLabel", font=("Segoe UI Semibold", 12)).grid(row=13, column=0, columnspan=3, sticky="w")

        self.zoom_enabled = tk.BooleanVar(value=False)
        ttk.Checkbutton(side, text="Enable zoom before recognition", variable=self.zoom_enabled, command=self.toggle_zoom).grid(row=14, column=0, columnspan=3, sticky="w", pady=(6, 0))

        self.zoom_factor = tk.DoubleVar(value=1.0)
        ttk.Label(side, text="Zoom factor", style="Card.TLabel").grid(row=15, column=0, sticky="w", pady=(10, 0))
        ttk.Scale(side, from_=1.0, to=8.0, variable=self.zoom_factor, command=lambda _v: self.on_zoom_factor_changed()).grid(row=16, column=0, columnspan=3, sticky="ew")
        self.zoom_status_var = tk.StringVar(value="Zoom OFF")
        ttk.Label(side, textvariable=self.zoom_status_var, style="Card.TLabel").grid(row=15, column=1, columnspan=2, sticky="e", pady=(10, 0))

        self.zoom_x = tk.DoubleVar(value=0.5)
        self.zoom_y = tk.DoubleVar(value=0.5)
        ttk.Label(side, text="Horizontal center", style="Card.TLabel").grid(row=17, column=0, sticky="w", pady=(8, 0))
        ttk.Scale(side, from_=0.0, to=1.0, variable=self.zoom_x, command=lambda _v: self.push_controls()).grid(row=18, column=0, columnspan=3, sticky="ew")
        ttk.Label(side, text="Vertical center", style="Card.TLabel").grid(row=19, column=0, sticky="w", pady=(8, 0))
        ttk.Scale(side, from_=0.0, to=1.0, variable=self.zoom_y, command=lambda _v: self.push_controls()).grid(row=20, column=0, columnspan=3, sticky="ew")

        pan = ttk.Frame(side, style="Card.TFrame")
        pan.grid(row=21, column=0, columnspan=3, sticky="ew", pady=(8, 0))
        for c in range(3):
        ...
        ttk.Checkbutton(side, text="Draw unknown faces", variable=self.draw_unknown, command=self.push_controls).grid(row=22, column=0, columnspan=3, sticky="w", pady=(12, 0))
        ttk.Checkbutton(side, text="Periodic grid search / slower high recall", variable=self.grid_search, command=self.push_controls).grid(row=23, column=0, columnspan=3, sticky="w", pady=(4, 0))
        ttk.Checkbutton(side, text="Hide confirmed known faces", variable=self.hide_confirmed_known, command=self.push_controls).grid(row=24, column=0, columnspan=3, sticky="w", pady=(4, 0))
        ttk.Label(side, text="Confirmed known similarity", style="Card.TLabel").grid(row=25, column=0, sticky="w", pady=(8, 0))
        ttk.Scale(side, from_=0.36, to=0.80, variable=self.confirmed_similarity, command=lambda _v: self.push_controls()).grid(row=26, column=0, columnspan=3, sticky="ew")
        ttk.Checkbutton(side, text="Stop repeated unknown faces", variable=self.hide_confirmed_unknown, command=self.push_controls).grid(row=27, column=0, columnspan=3, sticky="w", pady=(8, 0))
        ttk.Label(side, text="Stop Unknown after detections", style="Card.TLabel").grid(row=28, column=0, sticky="w", pady=(8, 0))
        ttk.Scale(side, from_=3, to=30, variable=self.confirmed_unknown_frames, command=lambda _v: self.push_controls()).grid(row=29, column=0, columnspan=3, sticky="ew")
        ttk.Checkbutton(side, text="Skip recognition for resolved faces", variable=self.skip_resolved_recognition, command=self.push_controls).grid(row=30, column=0, columnspan=3, sticky="w", pady=(8, 0))
        ttk.Label(side, text="Max SFace recognitions/frame (0 = all)", style="Card.TLabel").grid(row=31, column=0, sticky="w", pady=(8, 0))
        ttk.Scale(side, from_=0, to=40, variable=self.max_recognitions_per_frame, command=lambda _v: self.push_controls()).grid(row=32, column=0, columnspan=3, sticky="ew")
        ttk.Label(side, text="Stop known after detections", style="Card.TLabel").grid(row=33, column=0, sticky="w", pady=(8, 0))
        ttk.Scale(side, from_=0, to=300, variable=self.known_stop_after_detections, command=lambda _v: self.push_controls()).grid(row=34, column=0, columnspan=3, sticky="ew")
        ttk.Label(side, text="0 = disabled. 150 is recommended for classroom tests.", style="Muted.TLabel").grid(row=35, column=0, columnspan=3, sticky="w", pady=(8, 0))

        ttk.Label(
            side,
            text="Start/Stop controls are now fixed in the top bar. For video files, use Start/Stop plus the Pause video and Video speed controls in the top bar.",
            style="Card.TLabel",
            wraplength=340,
        ).grid(row=36, column=0, columnspan=3, sticky="w", pady=(18, 0))
    ...
    def _build_reports_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(known_header, text="Known persons", style="Title.TLabel").grid(row=0, column=0, sticky="w")
        ttk.Button(known_header, text="Delete ALL known faces", command=self.delete_all_known_faces).grid(row=0, column=1, sticky="e")
        ...
        ttk.Label(unknown_title_bar, text="Unknown review", style="Title.TLabel").grid(row=0, column=0, sticky="w")
        ttk.Label(
            unknown_title_bar,
        ...
        ttk.Button(unknown_tools, text="Export unknown + labeled faces", command=self.export_unknown_faces_package).grid(row=2, column=0, columnspan=2, sticky="ew", pady=(7, 0), padx=(0, 4))
        ttk.Button(unknown_tools, text="Import unknown faces", command=self.import_unknown_faces_package).grid(row=2, column=2, columnspan=2, sticky="ew", pady=(7, 0))
        ttk.Button(unknown_tools, text="Delete records without pictures", command=self.delete_unknowns_without_pictures).grid(row=3, column=0, columnspan=4, sticky="ew", pady=(7, 0))
    ...
    def _selected_playback_speed(self) -> float:
        raw = str(getattr(self, "playback_speed_choice", tk.StringVar(value="1x")).get() or "1x").strip().lower()
    ...
    def on_playback_speed_changed(self) -> None:
        if self.worker:
            self.worker.update_controls(playback_speed=self._selected_playback_speed())
        self._refresh_video_playback_controls()
