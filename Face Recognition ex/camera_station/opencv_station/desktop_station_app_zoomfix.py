...
class UnknownRegistry:
    ...
    def delete_unlabeled_tracks(self) -> Tuple[int, int]:
        """Delete all unknown tracks that have not been assigned a label.

        Returns:
            (deleted_tracks, deleted_images)
        """
        with self.lock:
            track_ids = [
                str(track_id)
                for track_id, track in self.tracks.items()
                if not str(track.get("assigned_label", "") or "").strip()
            ]
            if not track_ids:
                return 0, 0
            ...
            def delete_file_safely(path: Path) -> None:
                ...
                try:
                    ...
                    # Do not block the registry cleanup because one image is locked
                    # or already deleted.
                    pass
            ...
            for track_id in track_ids:
                ...
                for capture in track.get("captures", []):
                    ...
                    if rel_path:
                        delete_file_safely(self.base_dir / rel_path)

                # Also remove old/orphaned files that follow the default naming
                # convention but may not be present in the JSON capture list.
                try:
                    for image_path in self.images_dir.glob(f"{track_id}_*.jpg"):
                        delete_file_safely(image_path)
                except Exception:
                    pass
            ...
            self.save()
            return len(track_ids), deleted_images
...
class DesktopStationApp(tk.Tk):
    def __init__(self):
        ...
        self.latest_meta = {}
        self.unknown_sort = tk.StringVar(value="Last seen newest")
    ...
    def _build_reports_tab(self, parent: ttk.Frame) -> None:
        ...
        unknown_card.rowconfigure(1, weight=1)
        unknown_card.columnconfigure(0, weight=1)

        unknown_tools = ttk.Frame(unknown_card, style="Card.TFrame")
        unknown_tools.grid(row=0, column=0, sticky="ew", pady=(0, 8))
        unknown_tools.columnconfigure(1, weight=1)
        ttk.Label(unknown_tools, text="Sort by", style="Card.TLabel").grid(row=0, column=0, sticky="w")
        unknown_sort_box = ttk.Combobox(
            unknown_tools,
            textvariable=self.unknown_sort,
            state="readonly",
            values=(
                "Last seen newest",
                "Last seen oldest",
        ...
        unknown_sort_box.grid(row=0, column=1, sticky="w", padx=(8, 8))
        unknown_sort_box.bind("<<ComboboxSelected>>", lambda _event: self.refresh_reports())
        ttk.Button(
            unknown_tools,
            text="Delete all unlabeled",
            command=self.delete_unlabeled_unknowns,
        ...
        review_bar.grid(row=2, column=0, sticky="ew", pady=(10, 0))
    ...
    def _sorted_unknown_rows(self, rows: List[dict]) -> List[dict]:
        rows = list(rows or [])
        sort_mode = self.unknown_sort.get() if hasattr(self, "unknown_sort") else "Last seen newest"

        if sort_mode == "Last seen oldest":
            return sorted(
                rows,
                key=lambda row: (self._date_sort_value(row.get("last_seen")), str(row.get("id", ""))),
            )

        if sort_mode == "Detections high":
            return sorted(
                rows,
                key=lambda row: (
                    int(row.get("detections", 0) or 0),
                    self._date_sort_value(row.get("last_seen")),
                    str(row.get("id", "")),
                ),
                reverse=True,
            )

        if sort_mode == "Detections low":
            return sorted(
                rows,
                key=lambda row: (
                    int(row.get("detections", 0) or 0),
        ...
        if sort_mode == "Created newest":
            return sorted(
                rows,
                key=lambda row: (self._date_sort_value(row.get("created_at")), str(row.get("id", ""))),
                reverse=True,
            )

        if sort_mode == "Created oldest":
            return sorted(
                rows,
                key=lambda row: (self._date_sort_value(row.get("created_at")), str(row.get("id", ""))),
            )

        # Default: newest last-seen first.
        return sorted(
            rows,
            key=lambda row: (self._date_sort_value(row.get("last_seen")), str(row.get("id", ""))),
            reverse=True,
    ...
    def refresh_reports(self, meta: Optional[dict] = None) -> None:
        ...
        unknown_rows = self._sorted_unknown_rows(meta.get("unknown_report", self.registry.snapshot()))
        ...
        for row in unknown_rows:
            ...
            visible_unknown_iids.append(iid)
        for iid in unknown_existing:
            self.unknown_tree.delete(iid)
        for index, iid in enumerate(visible_unknown_iids):
            if self.unknown_tree.exists(iid):
                self.unknown_tree.move(iid, "", index)
    ...
    def delete_unlabeled_unknowns(self) -> None:
        current_rows = self.registry.snapshot()
        unlabeled_count = sum(1 for row in current_rows if not str(row.get("assigned_label", "") or "").strip())
        if unlabeled_count <= 0:
            messagebox.showinfo("Nothing to delete", "There are no unlabeled unknown tracks to delete.")
            return

        if not messagebox.askyesno(
            "Delete unlabeled unknowns",
            f"This will delete {unlabeled_count} unlabeled unknown track(s) and their saved pictures.\n\n"
            "Tracks that already have an assigned label will be kept.\n\n"
            "Continue?",
        ):
            return

        try:
            deleted_tracks, deleted_images = self.registry.delete_unlabeled_tracks()

            if self.latest_meta:
                self.latest_meta["unknown_report"] = self.registry.snapshot()

            current_selection = self.unknown_tree.selection()
            if current_selection:
                self.unknown_tree.selection_remove(*current_selection)
            self.preview_photo = None
            ...
            self.refresh_reports({
                "known_report": (self.latest_meta or {}).get("known_report", []),
                "unknown_report": self.registry.snapshot(),
            })
            messagebox.showinfo(
                "Unlabeled unknowns deleted",
                f"Deleted {deleted_tracks} unlabeled track(s) and {deleted_images} picture file(s).",
            )
        except Exception as exc:
            messagebox.showerror("Could not delete unlabeled unknowns", str(exc))
