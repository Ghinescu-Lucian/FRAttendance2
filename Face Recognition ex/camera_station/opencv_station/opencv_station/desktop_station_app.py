...
def configure_opencv_rtsp_environment() -> None:
    """Configure OpenCV/FFmpeg before the first VideoCapture is created.

    HiLook/Hikvision RTSP streams are much more stable over TCP than UDP.
    The discardcorrupt/low-delay options also prevent the reader from spending
    too much time on damaged H.264 packets when the stream hiccups.
    Users can still override this from PowerShell by setting
    OPENCV_FFMPEG_CAPTURE_OPTIONS before starting the app.
    """
...
configure_opencv_rtsp_environment()
