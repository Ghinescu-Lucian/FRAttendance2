...
GPU install, after you already have a compatible NVIDIA/CUDA/cuDNN environment:
  py -m pip uninstall -y onnxruntime
  py -m pip install insightface onnxruntime-gpu opencv-python numpy

Run:
  py variant_02_insightface_buffalo.py --subjects-dir images --model-name buffalo_l --device auto
"""
...
def choose_providers(device: str) -> Tuple[List[str], str]:
    ...
    if device == "cuda":
        ...
        print("[WARN] CUDA requested, but ONNX Runtime does not expose CUDAExecutionProvider. Falling back to CPU.")
        print(f"[WARN] Available ONNX Runtime providers: {providers or 'unknown'}")
        return ["CPUExecutionProvider"], "cpu"
...
def create_app(model_name: str, det_size: int, device: str):
    FaceAnalysis = import_insightface()
    providers, effective_device = choose_providers(device)

    print_header("Variant 02 - InsightFace")
    print(f"Requested device: {device}")
    print(f"Effective device: {effective_device}")
    print(f"ONNX Runtime providers: {get_onnxruntime_providers() or 'unknown'}")
    print(f"Using providers: {providers}")

    app = FaceAnalysis(name=model_name, providers=providers)
    ctx_id = 0 if effective_device == "cuda" else -1
    app.prepare(ctx_id=ctx_id, det_size=(det_size, det_size))
    return app, effective_device
...
def main() -> None:
    ...
    parser.add_argument("--device", choices=["auto", "cpu", "cuda"], default="auto", help="auto uses CUDA if ONNX Runtime exposes CUDAExecutionProvider.")
    ...
    configure_opencv_threads()
    app, effective_device = create_app(args.model_name, args.det_size, args.device)
    ...
    while True:
        ...
        cv2.putText(
            frame,
            f"InsightFace {args.model_name} [{effective_device}] | faces={len(faces)} | fps={current_fps:.1f} | ESC exit",
            (20, 35),
