...
# Required by current DeepFace/RetinaFace stacks when TensorFlow ships Keras 3.
...
def configure_tensorflow_before_import(device: str) -> None:
    if device == "cpu":
        os.environ["CUDA_VISIBLE_DEVICES"] = "-1"


def import_tensorflow_for_gpu_check(device: str):
    try:
        import tensorflow as tf
    except Exception as exc:
        print(f"[WARN] Could not import TensorFlow before DeepFace: {exc}")
        return None, "unknown"

    gpus = tf.config.list_physical_devices("GPU")

    if device == "cuda" and not gpus:
        print("[WARN] CUDA requested, but TensorFlow sees no GPU. DeepFace will probably run on CPU.")
...
def import_deepface():
    try:
        ...
        raise RuntimeError(
            ...
            "  py -m pip install deepface tensorflow tf-keras opencv-python numpy"
        ) from exc
...
def main() -> None:
    ...
    parser.add_argument("--device", choices=["auto", "cpu", "cuda"], default="auto", help="TensorFlow uses GPU automatically if visible. cpu hides GPU.")
    ...
    configure_tensorflow_before_import(args.device)
    configure_opencv_threads()
    tf, effective_device = import_tensorflow_for_gpu_check(args.device)

    print_header("Variant 04 - DeepFace")
    print(f"Requested device: {args.device}")
    print(f"Effective TensorFlow device guess: {effective_device}")
    if tf is not None:
    ...
    while True:
        ...
        cv2.putText(
            frame,
            f"DeepFace {args.model_name} [{effective_device}] | cached_faces={len(last_results)} | fps={current_fps:.1f} | ESC exit",
            (20, 35),
