...
# 3. When a known person is recognized, mark attendance once.
# 4. Save one proof photo.
# 5. Ignore the same person for a cooldown so the system can look for others.
#
...
WINDOW_NAME = "Fast Attendance Face Recognition"
...
# Save policy:
# - Save only ONE image file per capture event.
# - Save maximum two photos for each recognized person/name.
...
def save_person_photo(name, zoomed_frame):
    """
    Saves one centered/zoomed image of the recognized person.

    The program intentionally does not save both a zoomed image and a face crop.
    This avoids duplicate files for the same recognition event.
    """
...
def mark_attendance(name, photo_path):
    """
    Append one attendance event to attendance.csv.
    Format: timestamp, name, proof_photo_path
    """
...
def main():
    ...
    while True:
        ...
        if target_face is not None:
            ...
            # After attendance was marked, keep the crop on the person briefly,
            # then release lock so the camera can search for another student.
        ...
        if key in (ord("r"), ord("R")):
            ...
            stable_count = 0
            last_candidates = []
            # The saved photo and attendance counters are intentionally NOT reset.
