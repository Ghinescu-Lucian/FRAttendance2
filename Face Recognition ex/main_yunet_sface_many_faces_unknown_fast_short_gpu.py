...
# GPU acceleration for YuNet/SFace inference.
# This uses OpenCV DNN CUDA, not TensorFlow. It requires an OpenCV build compiled
# with CUDA + cuDNN. The normal pip package opencv-contrib-python is CPU-only.
USE_OPENCV_DNN_CUDA = True
USE_CUDA_FP16 = True  # faster on many NVIDIA GPUs; falls back to FP32 CUDA if unsupported
PRINT_OPENCV_CUDA_INFO = True
...
def _opencv_build_has_cuda():
    """Return True only when the loaded cv2 binary was actually built with CUDA."""
    try:
    ...
    return any(marker in build_info for marker in cuda_markers)
...
def get_dnn_backend_target():
    """
    Select the best OpenCV DNN backend for YuNet/SFace.

    Important:
      - cv2.dnn.DNN_BACKEND_CUDA existing is not enough.
      - The loaded OpenCV binary must be compiled with CUDA.
      - A CUDA-capable NVIDIA GPU and driver must be visible.
    """
    cpu_backend = getattr(cv2.dnn, "DNN_BACKEND_OPENCV", 3)
    cpu_target = getattr(cv2.dnn, "DNN_TARGET_CPU", 0)

    if not USE_OPENCV_DNN_CUDA:
        return cpu_backend, cpu_target, "CPU / OpenCV DNN"

    has_dnn_cuda_constants = hasattr(cv2.dnn, "DNN_BACKEND_CUDA") and hasattr(cv2.dnn, "DNN_TARGET_CUDA")
    has_cuda_build = _opencv_build_has_cuda()
    cuda_devices = _cuda_device_count()

    if has_dnn_cuda_constants and has_cuda_build and cuda_devices > 0:
        backend = cv2.dnn.DNN_BACKEND_CUDA
        if USE_CUDA_FP16 and hasattr(cv2.dnn, "DNN_TARGET_CUDA_FP16"):
            target = cv2.dnn.DNN_TARGET_CUDA_FP16
            label = f"CUDA / OpenCV DNN FP16 ({cuda_devices} device(s))"
        else:
            target = cv2.dnn.DNN_TARGET_CUDA
            label = f"CUDA / OpenCV DNN FP32 ({cuda_devices} device(s))"
        return backend, target, label

    if PRINT_OPENCV_CUDA_INFO:
        print("[GPU] CUDA requested, but OpenCV DNN CUDA is not usable in this Python environment.")
        print(f"[GPU] DNN CUDA constants present: {has_dnn_cuda_constants}")
        print(f"[GPU] OpenCV built with CUDA: {has_cuda_build}")
        print(f"[GPU] CUDA devices visible to OpenCV: {cuda_devices}")
        print("[GPU] Falling back to CPU. This is expected with normal pip opencv-contrib-python.")

    return cpu_backend, cpu_target, "CPU / OpenCV DNN fallback"


def create_detector(input_size: Tuple[int, int] = (320, 320), backend_id=None, target_id=None):
    if backend_id is None or target_id is None:
        backend_id, target_id, _ = get_dnn_backend_target()
    ...
    try:
        ...
        print("[GPU] This OpenCV Python binding does not accept backend/target for FaceDetectorYN. Using default CPU constructor.")
        return cv2.FaceDetectorYN_create(
            str(YUNET_MODEL),
            "",
            input_size,
...
def create_recognizer(backend_id=None, target_id=None):
    if backend_id is None or target_id is None:
        backend_id, target_id, _ = get_dnn_backend_target()
    ...
    try:
        ...
        print("[GPU] This OpenCV Python binding does not accept backend/target for FaceRecognizerSF. Using default CPU constructor.")
...
def main():
    ...
    backend_id, target_id, inference_backend_label = get_dnn_backend_target()
    print(f"Inference backend: {inference_backend_label}")

    detector = create_detector(backend_id=backend_id, target_id=target_id)
    recognizer = create_recognizer(backend_id=backend_id, target_id=target_id)
